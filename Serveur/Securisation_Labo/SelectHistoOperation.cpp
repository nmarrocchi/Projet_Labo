...
{
	QSqlQuery query;
	query.exec("SELECT security.room, security.byte, historical.statut, historical.date FROM historical, security WHERE security.idSecurity = historical.idSecurity ORDER BY date DESC LIMIT 50");
	...
	query.exec("SELECT passage.idCard, user.mail, passage.date FROM `passage`, `user` WHERE passage.idCard = user.idCard ORDER BY date DESC LIMIT 50");
